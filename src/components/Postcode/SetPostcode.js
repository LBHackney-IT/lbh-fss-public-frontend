...
const SetPostcode = () => {
  ...
  async function submitForm({ postcode }) {
    ...
    if (validPostcode) {
      ...
      let push =
        "?" + new URLSearchParams(prevUrlParamsArrayLast).toString().replace(/%2C/g, "+");
      ...
      if (prevUrl.length !== 0 && prevUrlParams.length !== 0) {
        ...
        if (
          ...
          prevUrlParamsArrayLast.support_service !== undefined
        ) {
          //
        } else {
          // catch all for list services
          const ListServicesSearchObj = [prevUrlParamsArrayLast].find(
            (ListServicesSearchObj) => ListServicesSearchObj.service_search,
          );
          let ListServicesPostcodeObj = [prevUrlParamsArrayLast].find(
