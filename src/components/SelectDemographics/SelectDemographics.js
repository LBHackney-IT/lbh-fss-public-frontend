...
const SelectDemographics = () => {
  ...
  let categoryExplorerObj,
    listServicesSearchObj,
    listServicesPostcodeObj = null;
  if (urlParams !== 0) {
    categoryExplorerObj = [urlParams].find(
      (categoryExplorerObj) => categoryExplorerObj.category_explorer,
    );
    listServicesSearchObj = [urlParams].find(
      (listServicesSearchObj) => listServicesSearchObj.service_search,
    );
    listServicesPostcodeObj = [urlParams].find(
  ...
  useEffect(() => {
    async function fetchData() {
      const getDemographics = await GetTaxonomies.retrieveTaxonomies({
  ...
  async function submitForm() {
    ...
    let push =
      "?" +
      new URLSearchParams(urlParams).toString().replace(/%2C/g, "+").replace(/%2B/g, "+");
