...
const Back = () => {
  ...
  const select = () => {
    ...
    if (prevUrl.length !== 0 && prevUrlParams !== 0) {
      for (const [key, value] of Object.entries(urlParams)) {
        ...
        if (key == "support_service" && value !== "") {
          if (prevUrl.length !== 0) {
            ...
            const categoryExplorerObj = prevUrlParams.find(
              (categoryExplorerObj) => categoryExplorerObj.category_explorer,
            );
            const listServicesSearchObj = prevUrlParams.find(
              (listServicesSearchObj) => listServicesSearchObj.service_search,
            );
            const listServicesPostcodeObj = prevUrlParams.find(
              (listServicesPostcodeObj) => listServicesPostcodeObj.postcode,
            );
      ...
      const selectCategoriesObj = [urlParams].find(
        (selectCategoriesObj) => selectCategoriesObj.select_categories,
      );
      const demographicObj = [urlParams].find(
        (demographicObj) => demographicObj.demographic,
      );
      const categoriesObj = [urlParams].find((categoriesObj) => categoriesObj.categories);
      const categoryExplorerObj = [urlParams].find(
        (categoryExplorerObj) => categoryExplorerObj.category_explorer,
      );
      const listServicesSearchObj = [urlParams].find(
        (listServicesSearchObj) => listServicesSearchObj.service_search,
      );
      const listServicesPostcodeObj = [urlParams].find(
        (listServicesPostcodeObj) => listServicesPostcodeObj.postcode,
      );
      if (selectDemographicsObj || selectCategoriesObj) {
        ...
        if (categoryExplorerObj) {
          params = {
            category_explorer: categoryExplorerObj.category_explorer,
        ...
        if (listServicesSearchObj || listServicesPostcodeObj) {
          if (
            listServicesSearchObj !== undefined &&
            listServicesSearchObj.service_search !== undefined
          ) {
      ...
      push =
        "?" +
        new URLSearchParams(params).toString().replace(/%2C/g, "+").replace(/%2B/g, "+");
      push = push.replaceAll("=undefined", "");
    }
